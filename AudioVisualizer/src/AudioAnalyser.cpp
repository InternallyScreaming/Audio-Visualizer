...
AudioAnalyser::AudioAnalyser()
    ...
    , m_numSpectrumAverages(0) //added by Tiffany
    , m_waveformLeft( NULL )
...
//Added by Tiffany
void AudioAnalyser::drawWaveformHex(int _x, int _y, int _x2, int _y2, int _width, int _height, int _rot)
{
    float sampleStepX = (float)_width / m_waveformNumSamples;
    ofPushMatrix();
        ...
        ofBeginShape();
                ...
                for (int i = 0; i < m_waveformNumSamples; i++) {
                    ...
                    ofVertex(sampleStepX * i + _x, -m_waveformMix[i] * halfHeight + _y);
                }
                ofVertex(_x2, _y2);
...
void AudioAnalyser::drawLinearAverages( int _x, int _y, int _width, int _height )
{
    ...
    if ( mouseY > _y && mouseY < _y + _height )
    {
        ...
        if ( mouseX > _x && mouseX < _x + _width )
        {
            ...
            m_font.drawString("(" + ofToString(index) + ") Freq: " + ofToString(index * freqStep) + " to " + ofToString((index + 1) * freqStep) + ",\nValue: " + ofToString(val * 1000.0f), mouseX + 8, mouseY - 50);
        }
    }
}
...
void AudioAnalyser::calcSpectrumAverages() //Added by Tiffany Wiebe
{
    float freqStep = (m_sampleRate / 2.0f) / m_fft.getBinSize() * m_numSpectrumBins;
    float f = 0.0f;

    for (int i = 0; i < m_numSpectrumBins; ++i)
    {
        m_spectrumMix[i] = calcAverage(f, f + freqStep) * 0.5f + m_spectrumMix[i] * 0.5f;
        f += freqStep;
    }
}
