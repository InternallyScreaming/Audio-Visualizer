...
public:
    ...
    void    drawWaveformHex(int _x, int _y, int _x2, int _y2, int _width, int _height, int _rot);           //Added by Tiffany
    ...
    //Added by Tiffany Wiebe
    inline float   getSpectrum(int _binIndex) const
    {
        if (_binIndex < 0 || _binIndex >= m_numSpectrumBins)
        {
            return 0;
        }
        return m_spectrumMix[_binIndex] * 1000.0f;
    }
...
private:
    ...
    float         * m_spectrumMix;
    int             m_numSpectrumAverages;          //Added by Tiffany Wiebe
